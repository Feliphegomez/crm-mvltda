...
class Site extends Route
	{
		...
		private function include_models_system()
			{
				foreach (glob("admin/plugins/system/models/*.php") as $f)
				{
					include_once($f);
				}
			}
		...
		public function get_view_plugin($plugin = null, $view = null)
			{
				$this->include_file("admin/plugins/{$plugin}/views/{$view}_view.php");
			}
		
		public function get_template_theme($template = null)
			{
				$this->include_file("admin/themes/".THEME_ACTIVE."/templates/{$template}.php");
				
			}
		
		public function get_controller_plugin($plugin = null, $controller = null)
			{
				$this->include_file("admin/plugins/{$plugin}/controllers/{$controller}_controller.php");
			}
		
		public function get_model_plugin($plugin = null, $model = null)
			{
				$this->include_file("admin/plugins/{$plugin}/models/{$model}_model.php");
			}
		...
		public function include_models_plugins()
		{
			...
			while (false !== ($n = readdir($g))) {
				if($n !== '.' && $n !== '..')
				{
					foreach (glob("admin/plugins/{$n}/models/*.php") as $f)
					{
						global $website;
						$website = $this;
						include_once($f);
		...
		{ if($filename !== null){ $this->include_file("admin/themes/".THEME_ACTIVE."/includes/{$filename}"); } }
		
		public function get_section_file($sectionfile)
		{ $this->include_file("admin/plugins/{$this->page->plugin}/modules/{$this->page->module}/sections/{$sectionfile}.php"); }
		
		public function get_assets_folder()
		{ 
			return $this->scheme.'://'.$this->server_name.':'.$this->server_port."/"."admin/themes/".THEME_ACTIVE."/";
		}
		
		public function get_assets_libs()
		{ 
			return $this->scheme.'://'.$this->server_name.':'.$this->server_port."/admin/libs/";
		}
		
		public function get_assets_global()
		{ 
			return $this->scheme.'://'.$this->server_name.':'.$this->server_port."/admin/global/";
		}
		
		public function get_section_active()
		{
			#if($this->plugin !== 'system' && $this->plugin !== 'admin')
				#$this->include_file("admin/global/navbar-admin.php");
			
			$this->include_file("admin/plugins/{$this->page->plugin}/modules/{$this->page->module}/sections/{$this->page->section}.php");
			if(MODE_DEBUG == true && MODE_DEBUG_FOOTER == true)
			{
		...
		public function get_global($filename = null) 
			{
				$this->include_file("admin/global/{$filename}.php");
			}
	}
